...
*  of ret2reg exploitation technique
*
***********************************/
...
static long vuln_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case CMD_CALL:
			...
			// obj->func();
			((void (*)(unsigned long))__x86_indirect_thunk_rdi)((unsigned long)obj->func);
			return 0;
		default:
			return -EINVAL;
	}
...
MODULE_DESCRIPTION("A vulnerable Linux kernel module to demonstrate the effectiveness of ret2reg exploitation technique.");
MODULE_VERSION("0.01");
